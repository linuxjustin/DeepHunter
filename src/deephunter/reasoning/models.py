...
class HypothesisPriority(str, Enum):
    """Priority level of an investigation hypothesis."""
...
class Hypothesis(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: f"hyp-{uuid4().hex[:12]}",
        description="Unique identifier (hyp-<12 hex chars>)",
    )
    title: str = Field(default="", description="Hypothesis title")
    description: str = Field(default="", description="Detailed description")
    bug_classes: list[BugClass] = Field(default_factory=list)
    technologies: list[str] = Field(
        default_factory=list,
        description="Technology strings relevant to this hypothesis",
    )
    ...
    rationale: str = Field(default="", description="Why this is worth investigating")
