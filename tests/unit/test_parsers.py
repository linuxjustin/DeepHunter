...
class TestParserRegistry:
    def test_register_and_get(self, registry: ParserRegistry) -> None:
        parser = registry.get(DocumentType.MARKDOWN)
        assert parser is not None
        assert isinstance(parser, MarkdownParser)

        parser = registry.get(DocumentType.HTML)
        assert parser is not None
        assert isinstance(parser, HTMLParser)

    def test_get_nonexistent(self, registry: ParserRegistry) -> None:
        assert registry.get(DocumentType.TEXT) is None

    def test_get_for_path(self, registry: ParserRegistry) -> None:
        parser = registry.get_for_path(Path("test.md"))
        assert isinstance(parser, MarkdownParser)

        parser = registry.get_for_path(Path("test.html"))
        assert isinstance(parser, HTMLParser)

        parser = registry.get_for_path(Path("test.unknown"))
        assert parser is None

    def test_get_for_extension(self, registry: ParserRegistry) -> None:
        parser = registry.get_for_extension("md")
        assert isinstance(parser, MarkdownParser)

        parser = registry.get_for_extension(".json")
        assert isinstance(parser, JSONParser)

    def test_list_types(self, registry: ParserRegistry) -> None:
    ...
    def test_clear(self, registry: ParserRegistry) -> None:
        registry.clear()
        assert registry.list_types() == []
        assert registry.get(DocumentType.MARKDOWN) is None
